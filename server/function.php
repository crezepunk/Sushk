	...
	echo "<div id = \"parent_div\">
										  <div id = \"div\">
											<p>Ваша заявка принята. <br>
											 В случае зачисления Вас на мастер-класс, Ваши данные отобразятся в таблице рейтинга<br>
											Ожидайте. 
											<a href=\"anketa.php\">Окей </a>
...
function addCardAdmin(){
	...
	$sql = mysql_query("INSERT INTO `students` (`fio`, `grade`, `email`, `phone`, `school_id`,  `active`) VALUES ('$fio', '$grade', '$email', '$phone', '$schoolsId', '$active')");
	$sql2 = mysql_query("SELECT id FROM `students` ORDER BY `id` DESC LIMIT 1");
	$result = mysql_fetch_array($sql2);
	$stud_id = $result["id"];
	//echo $stud_id;
	$sql3 = mysql_query("INSERT INTO `students_groups` (`student_id`, `group_id`, `startdate`) VALUES ('$stud_id', '$groups', '$now')");
	echo mysql_error();
	echo "<div id = \"parent_div\">
										  <div id = \"div\">
											<p>Вы успешно добавили нового студента</p>
...
function closeGroup(){
	...
	function addSubject($group_id){
		...
		$sql = mysql_query("INSERT INTO `subjects` (`name`, `description`, `group_id`, `date`) VALUES ('$name', '$description', '$group_id', '$days')");
		echo mysql_error();
		...
		$sql24 = "SELECT students_groups.student_id ,students_groups.group_id, students.fio as namestudent , groups.name as namegroup FROM students_groups 
							...
							if ($group_id){
								$sql24 .= " WHERE students_groups.group_id= $group_id";
							}
		...
		//Вставляем значения в таблицу tasks_students.
		$sql25 = "SELECT students_groups.student_id ,students_groups.group_id, students.fio as namestudent , groups.name as namegroup FROM students_groups 
														JOIN groups ON (groups.id=students_groups.group_id)
														JOIN students ON (students.id=students_groups.student_id)";
							if ($group_id){
								$sql25 .= " WHERE students_groups.group_id= $group_id";
							}
		$result_set24 = mysql_query($sql25);
		while ($result25 = mysql_fetch_array($result_set24)) {
			$student_id = $result25['student_id'];
			$idTask = mysql_query("SELECT * FROM  tasks  ORDER BY task_id DESC LIMIT 1");
			$resultIdTask = mysql_fetch_array($idTask);
			$Task_id23 = $resultIdTask['task_id'];
			//echo $Task_id23;
			$sql_task = mysql_query("INSERT INTO `tasks_students` (`student_id`, `task_id`, `mark`) VALUES ('$student_id', '$Task_id23', '0')");
			echo mysql_error();
		}		
	}
	function upSubject($subject_id){
		...
		$task_description = $_POST['task_description'];
		$up = "UPDATE subjects,tasks SET subjects.name='$name', subjects.date='$days', subjects.description='$description' , 
		tasks.task_name='$task_name', tasks.task_description='$task_description', tasks.task_weight='$task_weight'
		WHERE subjects.id='$subject_id' and tasks.subject_id='$subject_id'";
...
function addWork($studentIdForAddWork){
		$sql1 = mysql_query("SELECT `work` FROM `subjects_students` WHERE `student_id` = '$studentIdForAddWork'");
...
function addMark($mark,$task_idForMark,$studentIdForMark){ 
$sql = mysql_query("UPDATE `tasks_students` SET `mark` = '$mark' WHERE `student_id` = '$studentIdForMark' AND `task_id` = '$task_idForMark' "); 
echo mysql_error(); 
echo "OK"; 
}
